...
export default async (req: Request, context: Context) => {
  ...
  try {
    ...
    try {
      ...
      const watermarkPath = path.resolve(process.cwd(), 'public/assets/watermark.png');

      console.log('[WATERMARK_DEBUG] Loading physical watermark from path:', watermarkPath);
      const watermarkBuffer = fs.readFileSync(watermarkPath);
      console.log('[WATERMARK_DEBUG] Watermark buffer loaded. Byte length:', watermarkBuffer.length);
      console.log('[WATERMARK_DEBUG] Tiling pre-rendered watermark PNG pattern.');
